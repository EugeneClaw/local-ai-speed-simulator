...
| 2× DGX Spark + DSv4-Flash Q4 + MTP | 51 t/s | ~55 t/s ✓ |
...
**Quant speeds: decode scales by bandwidth, prefill doesn't move.** Decode is memory-bandwidth bound, so smaller quants are faster (fewer bytes per token) — but the measured anchors (IQ2 ≈ only 1.26× Q4 on a Spark) show ultra-low quants give back most of that advantage to dequant/compute overhead. The curve is strictly ordered and separated: IQ1 > IQ2 > IQ3 > Q4 > Q5 > Q6 > Q8. Prefill, by contrast, is **compute-bound** — the matmuls run at model precision regardless of storage quant — so changing quant doesn't change reading speed at all, and the agentic wall says the same thing on every quant (only hardware moves it).
